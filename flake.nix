    ...
    apps = forAllSystems (system: let
      ...
      applyScript = pkgs.writeShellScriptBin "apply" ''
        ...
        if [ "$OS" = "Darwin" ]; then
          ...
          echo "Applying Home Manager configuration (macOS)..."
          nix run path:${inputs.home-manager} -- switch --flake .#$USER@$HOSTNAME -b "backup-\$(date +%Y%m%d%H%M%S)"
        elif [ "$OS" = "Linux" ]; then
          if grep -q "NixOS" /etc/os-release 2>/dev/null; then
            echo "Applying NixOS configuration..."
            sudo nixos-rebuild switch --flake .#$HOSTNAME
            echo "Applying Home Manager configuration (NixOS)..."
            nix run path:${inputs.home-manager} -- switch --flake .#$USER@$HOSTNAME -b "backup-\$(date +%Y%m%d%H%M%S)"
          else
